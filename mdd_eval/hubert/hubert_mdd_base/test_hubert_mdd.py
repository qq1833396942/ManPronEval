...
MODEL_WEIGHTS = "best_hubert_mdd_model.pth" # 训练出的最优权重
...
def main():
    ...
    # 🚨 修改 2：使用新的模型类名实例化
    model = MDD_HuBERT_Attention_Model(num_pinyins=vocab_size, freeze_encoder=False, hubert_version=LOCAL_HUBERT_PATH)
    ...
    plt.title('MDD Confusion Matrix (HuBERT)', fontsize=16)
    plt.xlabel('Predicted Label (AI 判断)', fontsize=12)
    plt.ylabel('True Label (专家打分)', fontsize=12)
    
    save_path = "confusion_matrix_hubert.png"
