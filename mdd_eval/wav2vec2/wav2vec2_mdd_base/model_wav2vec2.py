...
class MDD_WAV2VEC2_Attention_Model(nn.Module):
    def __init__(self, num_pinyins, freeze_encoder=False, wav2vec2_version="facebook_wav2vec2_base_960"):
        super().__init__()
        print(f"🧬 Model 2 (Local + Attention) 初始化中... [wav2vec2 严格对照版]")
        ...
        if freeze_encoder:
            print("❄️ wav2vec2 Transformer 骨干网络已冻结")
            for param in self.wav2vec2.encoder.parameters():
                param.requires_grad = False

        # 获取音频特征维度 (wav2vec2 base 为 768)
    ...
    def forward(self, input_values, lengths, target_syllable_ids):
        ...
        # wav2vec2 的 CNN 下采样率也是 320，和 Wav2Vec2 完全一致
