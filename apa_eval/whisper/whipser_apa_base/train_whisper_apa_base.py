...
PARAMS = {
    "learning_rate": 1e-4,  # 严格对齐：底座已锁，1e-4 打分头完美适用
    "batch_size": 16,  # 严格对齐
    "epochs": 20,
    "patience": 5,
    # ⚠️ 更改了保存路径，防止和 HuBERT 的 checkpoint 发生冲突覆盖！
    "save_path": "best_whisper_base_v2_explicit.pth",
    "checkpoint_name": "whisper_base_apa_v2_checkpoint.pth"
}
...
def main():
    ...
    print(f"🚀 Whisper APA V2 架构启动 (显式交互版 + 严格对齐)！当前设备: {device}")
    ...
    # 完美对齐：只抓取 requires_grad=True 的参数（即我们没冻结的 MLP 和 Attention）
    ...
    for epoch in range(start_epoch, PARAMS["epochs"]):
        ...
        pd.DataFrame(history).to_csv("training_log_whisper_apa_v2.csv", index=False)
