...
# 🎯 指向你 Whisper-Base 训练跑出来的最优模型权重
MODEL_WEIGHTS = "best_whisper_base_v2_explicit.pth"
BATCH_SIZE = 16  # 测试时不需要算梯度，Base 模型显存占用较小，16 没问题


def run_test():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🔍 启动 Whisper-Base APA 测试引擎！当前设备: {device}")
    ...
    if not os.path.exists(MODEL_WEIGHTS):
        print(f"❌ 错误：未找到最优权重文件 {MODEL_WEIGHTS}，请先运行 train_whisper_apa.py")
        return
    ...
    # 6. 打印终极成绩单
    print("\n" + "=" * 60)
    print("🏆 Whisper-Base 测试集最终性能报告 (Test Set)")
    print("=" * 60)
    # 🚨 打印结果中加入 MAE
