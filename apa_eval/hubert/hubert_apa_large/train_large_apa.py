...
PARAMS = {
    "learning_rate": 1e-4,  
    "batch_size": 4,       # 🎯 对齐 Base：既然冻结了底座，大胆开到 16 稳定梯度
    "epochs": 20,           # 🎯 对齐 Base：改回 20
    "patience": 10,          # 🎯 对齐 Base：改回 5
    "save_path": "best_hubert_large_v2_explicit.pth",
    "checkpoint_name": "hubert_large_apa_v2_checkpoint.pth" 
}
...
def main():
    ...
    print(f"🚀 APA Large V2 架构启动！当前设备: {device}")

    # 1. 数据准备
    pinyin2id = build_pinyin_vocab([TRAIN_JSON, VAL_JSON, TEST_JSON])
    vocab_size = len(pinyin2id) + 1 # 🎯 对齐 Base：修复了之前 +2 导致的词表错位问题

    train_loader = DataLoader(APA_HuBERT_Dataset(TRAIN_JSON, pinyin2id=pinyin2id), 
                              batch_size=PARAMS["batch_size"], shuffle=True, collate_fn=collate_fn_apa) # 去掉了 drop_last
    ...
    # 2. 模型初始化
    model = APA_HuBERT_Large_Model(num_pinyins=vocab_size, hubert_version=LOCAL_HUBERT_PATH).to(device)
    ...
    for epoch in range(start_epoch, PARAMS["epochs"]):
        ...
        pd.DataFrame(history).to_csv("training_log_apa_large_v2.csv", index=False)
