...
class APA_HuBERT_Large_Model(nn.Module):
    def __init__(self, num_pinyins, embed_dim=128, hubert_version="facebook/hubert-large-ls960-ft"):
        super().__init__()
        print(f"🧬 APA 考官模型 (Large 版 + 显式交互 + 冻结底座) 初始化中...")

        self.hubert = HubertModel.from_pretrained(hubert_version, output_hidden_states=True)
        # 关闭遮盖防止短音频报错
