...
def main():
    ...
    if not os.path.exists(MODEL_WEIGHTS):
        print(f"❌ 找不到权重文件 {MODEL_WEIGHTS}！请确认文件名。")
        return
        
    checkpoint = torch.load(MODEL_WEIGHTS, map_location=device)
    
    # 自动判断是纯权重还是带状态的字典，且使用 strict=False 兼容 LoRA
    ...
    # 7. 可视化 (画 3 张图)
    print("🎨 正在生成 3 张大模型打分回归分析图...")
    plot_regression(t_ini_10, p_ini_10, f'Large Initial Score (PCC={metrics["Initial (声母)"]["PCC"]:.3f})', 'apa_large_test_regression_initial.png')
    plot_regression(t_fin_10, p_fin_10, f'Large Final Score (PCC={metrics["Final (韵母)"]["PCC"]:.3f})', 'apa_large_test_regression_final.png')
    plot_regression(t_tot_10, p_tot_10, f'Large Total Score (PCC={metrics["Total (总分)"]["PCC"]:.3f})', 'apa_large_test_regression_total.png')
    ...
    results_df.to_csv("test_large_multiscore_results_detail.csv", index=False)
    print(f"✅ 详细多维度预测对比已保存至: test_large_multiscore_results_detail.csv")
