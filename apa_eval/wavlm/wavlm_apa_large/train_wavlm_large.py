...
# ==========================================
# 1. 核心配置：严格对照 HuBERT Large 实验
# 仅替换声学底座路径为本地 WavLM Large
# ==========================================
LOCAL_WAVLM_PATH = r""
...
PARAMS = {
    ...
    "epochs": 30,
    "patience": 5,
    "save_path": "best_wavlm_large_v2_explicit.pth",
    "checkpoint_name": "wavlm_large_apa_v2_checkpoint.pth"
}
...
def main():
    ...
    print(f"🚀 APA WavLM Large 严格对照实验启动！当前设备: {device}")
    ...
    # 2. 模型初始化
    model = APA_WavLM_Large_Model(num_pinyins=vocab_size, wavlm_version=LOCAL_WAVLM_PATH).to(device)
    ...
    for epoch in range(start_epoch, PARAMS["epochs"]):
        ...
        pd.DataFrame(history).to_csv("training_log_wavlm_large_v2.csv", index=False)
