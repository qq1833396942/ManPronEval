...
# ==========================================
# 1. 核心配置：WavLM Base 测试
# ==========================================
LOCAL_WAVLM_PATH = r""
TEST_JSON = 'metadata_test_apa.json'
MODEL_WEIGHTS = "best_wavlm_base_v2_explicit.pth"
...
def main():
    ...
    print(f"🚀 开始 APA WavLM Base 多维度测试！设备: {device}")
    ...
    # 3. 初始化模型并加载权重
    model = APA_WavLM_Base_Model(num_pinyins=vocab_size, wavlm_version=LOCAL_WAVLM_PATH)
    ...
    print("\n" + "=" * 50)
    print("🏆 APA WavLM Base 多维度测试集最终成绩单 🏆")
    print("=" * 50)
    ...
    plot_regression(
        ...
        'wavlm_test_regression_initial.png'
    )
    plot_regression(
        t_fin_10, p_fin_10,
        f'Final Score (PCC={metrics["Final (韵母)"]["PCC"]:.3f})',
        'wavlm_test_regression_final.png'
    )
    plot_regression(
        t_tot_10, p_tot_10,
        f'Total Score (PCC={metrics["Total (总分)"]["PCC"]:.3f})',
        'wavlm_test_regression_total.png'
    )
    ...
    results_df.to_csv("wavlm_test_multiscore_results_detail.csv", index=False)
    print("✅ 详细预测对比已保存至: wavlm_test_multiscore_results_detail.csv")
