...
def test_best_model():
    ...
    BEST_WEIGHTS = os.path.join(CODE_DIR, "mtl_wavlm_base_best.pth")
    ...
    model = MultiTaskWavLMBase(model_path=LOCAL_MODEL_PATH, vocab_size=len(pinyin2id)).to(device)
    ...
    print("\n" + "═" * 65)
    print(f"{'🏆 WavLM-Base 单音节多任务模型最终评估报告':^58}")
    print("═" * 65)
