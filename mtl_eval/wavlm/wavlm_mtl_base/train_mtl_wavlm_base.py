...
from model_mtl_wavlm_base import MultiTaskWavLMBase
...
def train_and_validate_mtl():
    ...
    print(f"🔥 计算设备: {device} | WavLM-Base 严格对照单学习率模式 (1e-4)")
    ...
    model = MultiTaskWavLMBase(model_path=LOCAL_MODEL_PATH, vocab_size=vocab_size).to(device)
    ...
    for epoch in range(epochs):
        ...
        for t, p in zip(asr_true, asr_pred):
            ...
            seq_t = true_pinyin.split('_') if '_' in true_pinyin else (
                true_pinyin.split() if ' ' in true_pinyin else list(true_pinyin)
            )
            seq_p = pred_pinyin.split('_') if '_' in pred_pinyin else (
                pred_pinyin.split() if ' ' in pred_pinyin else list(pred_pinyin)
            )
        ...
        print(
            ...
            f"APA: {train_apa_loss/num_train_batches:.2f} | "
            f"MDD: {train_mdd_loss/num_train_batches:.2f} | "
            f"ASR: {train_asr_loss/num_train_batches:.2f}"
        )
        print(
            f"   📈 Val   Loss: {avg_val_loss:.4f}  | "
            f"🎯 APA PCC: {val_pcc_total:.4f} | "
            f"🎯 MDD F1: {val_mdd_f1:.4f} | "
            f"🎯 ASR Acc: {val_asr_acc:.4f} | "
            f"🎯 ASR PER: {val_asr_per:.4f}"
        )

        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            best_model_path = os.path.join(SAVE_DIR, "mtl_wavlm_base_best.pth")
